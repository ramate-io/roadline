//! From: https://github.com/zortax/leptos-resize/blob/main/src/lib.rs
...
pub fn ResizableSplit(
	...
	let on_mouse_move = move |ev: ev::MouseEvent| {
		...
		if let (Some(index), Some(container)) = (resizing_index.get(), container.get()) {
	...
	let grid_template_columns = move || match direction {
		SplitDirection::Row => {
			...
			grid_template
				.push_str(&format!("{:.2}%", 100.0 - percentages.get().iter().sum::<f64>()));
			grid_template
		}
	...
	let handle_style = move |index: usize| {
		let position = percentages.get().iter().take(index + 1).fold(0.0, |acc, &x| acc + x);
	...
	if index < new_percentages.len() {
		...
		if new_percentage >= prev_sum && new_percentage <= 100.0 - next_sum {
			...
			if index < new_percentages.len() - 1 {
				new_percentages[index + 1] += current_percentage - new_percentages[index];
			}
		}
	}
	new_percentages
