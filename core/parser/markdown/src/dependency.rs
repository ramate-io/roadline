...
impl DependencyParser {
    ...
    pub fn parse_dependencies(&self, section: &TaskSection) -> Result<Option<Vec<u8>>, MarkdownParseError> {
    ...
    /// - Empty set: "$\\emptyset$"
    fn parse_dependency_value(&self, value: &str) -> Result<Option<Vec<u8>>, MarkdownParseError> {
        ...
        let dependencies: Result<Vec<u8>, _> = value
            .split(',')
            .map(|dep| self.parse_single_dependency(dep.trim()))
            .collect();
    ...
    fn parse_single_dependency(&self, dep_str: &str) -> Result<u8, MarkdownParseError> {
        ...
        let number: u8 = number_str.parse()
            .map_err(|_| MarkdownParseError::InvalidDependencyReference {
                reference: dep_str.to_string(),
            })?;
    ...
    pub fn parse_dependency_line(&self, line: &str) -> Result<Option<Vec<u8>>, MarkdownParseError> {
        let line = line.trim();
    ...
    pub fn extract_task_id(&self, dep_ref: &str) -> Result<u8, MarkdownParseError> {
