...
use crate::range_algebra::{RangeAlgebra, Date};
...
impl PreGridAlgebra {
    ...
    pub fn compute(self) -> Result<GridAlgebra, GridAlgebraError> {
        ...
        for (task_id, stretch) in task_stretches {
            let lane_id = lane_assignments.get(&task_id)
        ...
        let max_x_axis = tasks.values()
            .map(|cell| cell.stretch().end())
            .max()
            .unwrap_or(0);

        let max_y_axis = tasks.values()
            .map(|cell| cell.lane_id())
            .max()
            .unwrap_or(0);

        let total_lanes = if max_y_axis == 0 && !tasks.is_empty() { 1 } else { max_y_axis as usize + 1 };
    ...
    fn determine_time_scale_unit(&self) -> Result<StretchUnit, GridAlgebraError> {
        ...
        let total_duration_seconds: u64 = spans.values()
            .map(|span| {
    ...
    fn calculate_task_stretches(&self, time_unit: StretchUnit) -> Result<HashMap<TaskId, Stretch>, GridAlgebraError> {
        let spans = self.range_algebra.spans();
        
        // Find the earliest start time to use as reference point
        let reference_time = spans.values()
        ...
        for (&task_id, span) in spans {
            ...
            let end_unit = ((end_timestamp - reference_time + unit_seconds - 1) / unit_seconds) as u8; // Ceiling division
    ...
    fn assign_lanes_dfs(&self, task_stretches: &HashMap<TaskId, Stretch>) -> Result<HashMap<TaskId, LaneId>, GridAlgebraError> {
        ...
        let root_widths: Vec<(TaskId, usize)> = roots.iter()
            .map(|&root| {
    ...
    fn count_overlapping_tasks(&self, tasks: &[TaskId], task_stretches: &HashMap<TaskId, Stretch>) -> usize {
        if tasks.len() <= 1 {
            return tasks.len();
        }
        
        // Sort tasks by start time
        let mut sorted_tasks: Vec<_> = tasks.iter()
            .filter_map(|&task_id| {
                task_stretches.get(&task_id).map(|stretch| (task_id, stretch))
            })
            .collect();
    ...
    fn dfs_assign_lane(
        ...
        let task_stretch = task_stretches.get(&task_id)
    ...
    fn find_available_lane(
        ...
        if preferred < occupancy.len() && !self.has_temporal_overlap(preferred, stretch_range, occupancy) {
            return preferred;
        }

        // Spiral outward: preferred±1, preferred±2, ...
        for offset in 1..=255 { // Max lanes limited by u8
            // Try lane below preferred
            if let Some(candidate) = preferred.checked_sub(offset) {
                if candidate < occupancy.len() && !self.has_temporal_overlap(candidate, stretch_range, occupancy) {
                    return candidate;
                }
            }
...
impl GridAlgebra {
    ...
    pub fn tasks_in_lane(&self, lane_id: u8) -> Vec<(&TaskId, &Cell)> {
        self.tasks.iter()
            .filter(|(_, cell)| cell.lane_id() == lane_id)
            .collect()
    }
...
mod tests {
    ...
    fn test_parallel_lane_assignment() -> Result<(), anyhow::Error> {
        ...
        assert_ne!(task2_lane, task3_lane, "Overlapping tasks T2 and T3 should be in different lanes");
