...
const OROAD_0_REPO_PATH: &str = "ramate-io/oac/oroad/oera-000-000-000-dulan/oroad-000-000-000/README.md";
...
mod integration_tests {
    ...
    async fn test_oroad_0_from_raw_url() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(task_titles.iter().any(|title| title.contains("T1")), "Should contain T1 task");
        assert!(task_titles.iter().any(|title| title.contains("T9")), "Should contain T9 task");
        Ok(())
    }
    ...
    async fn test_all_url_formats_produce_same_result() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(raw_roadline.graph().arena.tasks().len(), tree_roadline.graph().arena.tasks().len());
        assert_eq!(tree_roadline.graph().arena.tasks().len(), blob_roadline.graph().arena.tasks().len());
        assert_eq!(blob_roadline.graph().arena.tasks().len(), repo_roadline.graph().arena.tasks().len());
        
        // All should have the same task titles
        let raw_titles: Vec<String> = raw_roadline.graph().arena.tasks().iter()
            .map(|(_, task)| task.title().text.clone())
            .collect();
        let tree_titles: Vec<String> = tree_roadline.graph().arena.tasks().iter()
            .map(|(_, task)| task.title().text.clone())
            .collect();
        let blob_titles: Vec<String> = blob_roadline.graph().arena.tasks().iter()
            .map(|(_, task)| task.title().text.clone())
            .collect();
        let repo_titles: Vec<String> = repo_roadline.graph().arena.tasks().iter()
            .map(|(_, task)| task.title().text.clone())
            .collect();
    ...
    #[tokio::test]
    async fn test_oroad_0_task_dates() -> Result<(), Box<dyn std::error::Error>> {
        let source = GitHubSource::new();
        let roadline = source.from_raw_url(OROAD_0_RAW_URL).await?;
        let roadline = roadline.build()?;
        ...
        // All tasks should have start and end dates
        for (_, task) in tasks {
            assert!(task.range().start().point_of_reference().is_some(), "Task {} should have start date", task.title().text);
            assert!(task.range().end().duration().is_some(), "Task {} should have end date", task.title().text);
        }
        Ok(())
    }
}
...
mod error_handling_tests {
    ...
    async fn test_nonexistent_repository() {
        let source = GitHubSource::new();
        let result = source.from_raw_url("https://raw.githubusercontent.com/nonexistent/repo/main/README.md").await;
    ...
    async fn test_nonexistent_file() {
        let source = GitHubSource::new();
        let result = source.from_raw_url("https://raw.githubusercontent.com/ramate-io/oac/main/nonexistent.md").await;
...
mod content_validation_tests {
    ...
    async fn test_oroad_0_content_structure() {
        ...
        assert!(task_titles.iter().any(|title| title.contains("Push Towards Validation")), "Should contain T1");
        assert!(task_titles.iter().any(|title| title.contains("Validation and Accepting Contributions")), "Should contain T2");
        assert!(task_titles.iter().any(|title| title.contains("An Interlude")), "Should contain T9");
        
        // Check for subtasks
        let all_content: String = tasks.iter()
            .flat_map(|(_, task)| task.subtasks().iter())
            .map(|subtask| subtask.title().text.clone())
            .collect::<Vec<_>>()
            .join(" ");
        
        assert!(all_content.contains("Complete draft of OART-1: BFA"), "Should contain OART-1 subtask");
        assert!(all_content.contains("Begin gwrdfa implementation"), "Should contain gwrdfa subtask");
    }
